...
<style>
    ...
    .result-positive {
        background: linear-gradient(135deg,
            rgba(233,69,96,0.2), rgba(194,49,82,0.1));
    ...
    .result-negative {
        background: linear-gradient(135deg,
            rgba(46,213,115,0.2), rgba(39,174,96,0.1));
    ...
    .custom-divider {
        ...
        background: linear-gradient(90deg,
            transparent, #e94560, #53a8b6, transparent);
        border: none;
        margin: 30px 0;
        border-radius: 2px;
    }
...
@st.cache_resource
def load_artifacts():
    # Step 1: Model architecture banao
    _model = Sequential([
    ...
    # Step 2: Pickle se weights load karo
    with open('model_weights.pkl', 'rb') as f:
        weights = pickle.load(f)
    _model.set_weights(weights)

    # Step 3: Scaler load karo
    _scaler = joblib.load('scaler.pkl')

    return _model, _scaler


# ← GLOBAL SCOPE mein assign karo
...
with st.sidebar:
    ...
    for val, lbl in [("81%", "Recall (Sensitivity)"),
                     ("68%", "F1 Score"),
                     ("82%", "ROC AUC Score")]:
        st.markdown(f"""
        <div class="stat-card">
    ...
    st.markdown("""
    <div class="glass-card">
        <p style="color:#e0e0e0; line-height:2;">
        ⚡ TensorFlow / Keras<br>
        📊 Scikit-Learn<br>
...
with col2:
    ...
    bmi     = st.number_input("⚖️ BMI (kg/m²)",
...
# ==========================================
# PREDICTION — model & scaler GLOBAL hain
# ==========================================
if predict_clicked:
    ...
    # Scale karo  ← scaler global hai, koi error nahi
    input_scaled = scaler.transform(input_data)

    # Predict karo  ← model global hai, koi error nahi
    ...
    if prediction == 1:
        ...
        <div class="result-positive">
            ...
            Our AI model indicates a
            <b style="color:#e94560;">{prediction_prob*100:.1f}%</b>
            probability of diabetes. Please consult a healthcare
        ...
        <div class="result-negative">
            ...
            <b style="color:#2ed573;">{(1-prediction_prob)*100:.1f}%</b>
            probability of being diabetes-free.
...
<div class="footer">
    <p>
    ⚠️ <b>Medical Disclaimer:</b> This is an AI-powered predictive tool
    for educational purposes only. <b>NOT</b> a substitute for professional
    medical diagnosis. Always consult a qualified healthcare professional.
    </p>
    <p style="margin-top:15px;">
    🩺 Diabetes Prediction AI ·
    <a href="https://github.com/mahadiurrehman-pixel/diabetes-ann-model">
        GitHub
    </a> ·
